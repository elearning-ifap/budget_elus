...
const fresh=()=>({
 ...
 flips:[],indicatorQual:{},hypotheses:[],scenarios:[],simFinance:'mixte',simRevenue:1,simControl:false,vigilances:[],
 priorities:[],interlocDone:false,questions:[],exchangeCurrent:0,exchangeSteps:[0,0],exchangeMetrics:[{precision:0,instruction:0},{precision:0,instruction:0}],exchangeChoices:[[],[]],exchangeNotes:[[],[]],exchangeDone:false,
 finished:false
});
let state=fresh();
...
function compactModule(){
 return {v:7,b:state.badges,rs:state.repSeen,rc:state.repCurrent,as:state.actorsSeen,cp:state.cyclePlaced,co:state.cycleOrder,cm:state.cycleMode,pl:state.plan,
  ...
  cs:state.calcShown,ct:state.constats,rf:state.reflexDone,fl:state.flips,iq:state.indicatorQual,hy:state.hypotheses,sc:state.scenarios,
  sf:state.simFinance,sr:state.simRevenue,sx:state.simControl,vi:state.vigilances,pa:state.priorities,qu:state.questions,
  eq:state.exchangeCurrent,ex:state.exchangeSteps,em:state.exchangeMetrics,ec:state.exchangeChoices,en:state.exchangeNotes,ed:state.exchangeDone};
}
function restoreModule(c){if(!c||![3,4,5,6,7].includes(c.v))return;const seen=(c.rs||state.repSeen).filter(x=>Number(x)>=1&&Number(x)<=4),legacy=c.v<6;Object.assign(state,{badges:c.b||state.badges,repSeen:seen,repCurrent:c.rc??state.repCurrent,actorsSeen:c.as||state.actorsSeen,cyclePlaced:c.cp||state.cyclePlaced,cycleOrder:c.co||state.cycleOrder,cycleMode:c.cm||(c.cp&&c.cp.length?'build':state.cycleMode),plan:c.pl||state.plan,docNeedIndex:c.dni??state.docNeedIndex,docNeedDone:c.dnd||state.docNeedDone,docNeedOrder:c.dno||state.docNeedOrder,evolIndex:c.evi??state.evolIndex,evolDone:c.evd||state.evolDone,evolOrder:c.evo||state.evolOrder,dossierPiece:c.dsp||state.dossierPiece,dossierSelected:c.dss||state.dossierSelected,dossierDone:c.dsd||state.dossierDone,dossierTry:c.dst||state.dossierTry,dossierOrder:c.dso||state.dossierOrder,sections:c.se||state.sections,calcStep:c.ci??state.calcStep,calcShown:c.cs||state.calcShown,constats:c.ct||state.constats,reflexDone:c.rf??state.reflexDone,flips:c.fl||state.flips,indicatorQual:c.iq||state.indicatorQual,hypotheses:c.hy||state.hypotheses,scenarios:c.sc||state.scenarios,simFinance:c.sf||state.simFinance,simRevenue:c.sr??state.simRevenue,simControl:c.sx??state.simControl,vigilances:c.vi||state.vigilances,priorities:c.pa||state.priorities,questions:c.qu||state.questions,exchangeCurrent:legacy?0:(c.eq??0),exchangeSteps:legacy?[Number(c.ex)||0,0]:(c.ex||state.exchangeSteps),exchangeMetrics:legacy?[{precision:Number(c.ep)||0,instruction:Number(c.ei)||0},{precision:0,instruction:0}]:(c.em||state.exchangeMetrics),exchangeChoices:legacy?[Array.isArray(c.ec)?c.ec:[],[]]:(c.ec||state.exchangeChoices),exchangeNotes:legacy?[Array.isArray(c.en)?c.en:[],[]]:(c.en||state.exchangeNotes),exchangeDone:c.ed??state.exchangeDone});}
function save(){const ok=GABARIT.donnee('budget',compactModule());updateBadges();if(!ok)console.error('[BUDGET] Échec de persistance de l’état du module.');return ok;}
...
const MISSION_REQUIREMENTS={
  ...
  5:['bud-m5-priorites','bud-m5-interlocuteur','bud-m5-question','bud-m5-echanges-intro','bud-m5-commission']
};
...
function syncBottomNavigation(){const next=$('g-btn-suiv');if(!next)return;const p=(GABARIT.etat||{}).page;if(!SEQ.includes(p))return;const exchangeMode=p==='bud-m5-commission';next.style.display=exchangeMode?'none':'';next.setAttribute('aria-hidden',exchangeMode?'true':'false');if(exchangeMode){next.disabled=true;return}if(p==='g-post'){next.disabled=Object.keys(GABARIT.etat.post||{}).length<(D.COMP||[]).length;return}if(p==='bud-m5-question'){next.disabled=!questionsPretes();return}next.disabled=!isComplete(p)}
function updateBadges(){
 ...
 if($('final-badges')){
  const badgeTitles=["Maîtriser les repères", "Explorer le dossier", "Lire les équilibres", "Interpréter les indicateurs", "Préparer ses échanges"];
...
function chooseActor(key){state.actorsSeen=Array.from(new Set([...(state.actorsSeen||[]),key]));qa('.actor').forEach(b=>{const active=b.dataset.actor===key;b.classList.toggle('active',active);b.setAttribute('aria-pressed',active?'true':'false')});const a=ACTORS[key];qa('#actor-cycle [data-stage]').forEach(s=>{const related=a.stages.includes(s.dataset.stage);s.classList.toggle('related',related);s.classList.toggle('dim',!related);s.setAttribute('aria-current',related?'step':'false')});html($('actor-detail'),`<h4>${esc(a.title)}</h4><p>${esc(a.body)}</p><p class="muted"><strong>Étapes entourées dans le cycle :</strong> ${a.stages.map(s=>({prepare:'Préparer',execute:'Exécuter',exam:'Examiner',vote:'Voter',control:'Contrôler'}[s]||s)).join(' · ')}</p>`);text($('actor-counter'),`${state.actorsSeen.length}/5 acteurs explorés`);$('actor-next').disabled=state.actorsSeen.length<5;if(state.actorsSeen.length===5)complete('bud-m1-acteurs');save()}
...
const DOSSIER_FEEDBACK={
 rapport:'<strong>Pourquoi ces trois signaux ?</strong> Les charges progressent plus vite que les recettes, l’épargne brute recule et l’emprunt finance 27 % des investissements : ces trois éléments doivent être rapprochés. À l’inverse, une hausse des recettes ou un équilibre formel ne suffisent pas à conclure à une amélioration durable.',
 maquette:'<strong>Pourquoi ces trois signaux ?</strong> Le poids du personnel, la baisse de l’épargne brute et le rapprochement entre nouvel emprunt et capital remboursé éclairent la marge et la dette. La seule hausse des recettes ou un résultat nul ne démontrent pas la soutenabilité.',
 trajectoire:'<strong>Pourquoi ces trois signaux ?</strong> La baisse de l’épargne, la dégradation du désendettement et la poursuite de la trajectoire malgré la disparition de la dépense exceptionnelle se lisent ensemble. Une seule baisse de l’emprunt ou de l’investissement ne suffit pas à conclure.',
 invest:'<strong>Pourquoi ces trois signaux ?</strong> La signature des subventions, les 2,9 Md d’autofinancement des tranches futures et l’absence d’autorisation de programme conditionnent la suite. Le financement de N ne sécurise pas automatiquement N+1 et N+2.'
};
function checkDossier(){const key=state.dossierPiece||'rapport';if(state.dossierDone[key]){nextDossierPiece();return}const d=D.DOSSIER[key],sel=state.dossierSelected[key]||[];if(sel.length!==3)return;const ok=sel.every(i=>d.options[i][1]===1)&&d.options.filter(x=>x[1]===1).every(x=>sel.includes(d.options.indexOf(x)));if(ok){state.dossierDone[key]=true;qa('#signal-choices .choice.selected').forEach(b=>b.classList.add('correct'));feedback('signal-feedback','ok',`<strong>Pièce validée.</strong> ${DOSSIER_FEEDBACK[key]||'Les trois signaux structurants rejoignent votre carnet.'}`);if(Object.values(state.dossierDone).every(Boolean)){complete('bud-dossier');$('signal-check').textContent='Terminer l’analyse →'}else $('signal-check').textContent='Pièce suivante →'}else{state.dossierTry[key]=(state.dossierTry[key]||0)+1;const wrong=sel.filter(i=>d.options[i][1]===0);state.dossierSelected[key]=sel.filter(i=>d.options[i][1]===1);renderDossierSelection();wrong.forEach(i=>{const b=$('signal-choices').querySelector(`[data-idx="${i}"]`);if(b){b.classList.remove('selected','correct','wrong');b.classList.add('released-wrong');b.setAttribute('aria-pressed','false');b.disabled=true}});const hint=d.hints[Math.min(state.dossierTry[key]-1,d.hints.length-1)];feedback('signal-feedback','bad',`<strong>Pas encore.</strong> Le choix retiré allait au-delà de ce que la pièce permet d’établir. Vos bons signaux restent sélectionnés : choisissez seulement ce qui manque.<br><strong>Indice :</strong> ${esc(hint)}<br><strong>Réflexe transférable :</strong> distinguez toujours un chiffre observé de la conclusion qu’il faudrait encore documenter.`)}updateDossierProgress();save()}
...
function saveConstats(){if(isComplete('bud-m3-constats')){go('bud-m3-reflexe');return}const bad=state.constats.some(i=>$('constat-choices').querySelector(`[data-constat="${i}"]`).dataset.ok!=='1');if(bad){releaseWrongSelections('constat-choices','constat','constats',3,'constat-counter','constat-save','constats sélectionnés');feedback('constat-feedback','bad','<strong>Un constat allait trop loin.</strong> Une dépense exceptionnelle explique une partie de N−1, mais elle ne prouve pas que la trajectoire est maîtrisée : l’épargne baisse encore en N et la dette progresse. Le choix a été retiré ; conservez les observations factuelles et choisissez ce qui manque.');return}feedback('constat-feedback','ok','<strong>Constats conservés.</strong> Vous avez séparé trois faits établis — baisse de l’épargne nette, hausse de la dette et dépense exceptionnelle en N−1 — de leur interprétation. Le réflexe transférable est de documenter la cause avant de conclure.');complete('bud-m3-constats');$('constat-save').textContent='Continuer';renderCarnet();save()}

/* M4 */
function initFlips(){
 const update=()=>{const seen=(state.flips||[]).length,qualified=Object.keys(state.indicatorQual||{}).length;text($('flip-counter'),`${seen}/4 cartes · ${qualified}/4 indicateurs qualifiés`);$('flip-next').disabled=seen<4||qualified<4;if(seen===4&&qualified===4)complete('bud-m4-indicateurs')};
 ...
 const expected=['watch','watch','good','good'];
 const explanations=['Le tableau de bord expert classe l’épargne brute de 13 Md en point de vigilance. Le niveau reste positif, mais la baisse de 22 à 16 puis 13 Md impose d’en rechercher les causes.','Le tableau de bord expert classe le besoin à financer de 9 Md en point de vigilance. La question devient : quelles ressources couvriront ce besoin et avec quel degré de sécurité ?','Le repère expert situe une capacité de désendettement inférieure à 8 ans en situation saine : 7,3 ans est donc favorable. En revanche, la trajectoire 3,4 → 5,5 → 7,3 ans doit rester sous surveillance.','Le repère expert situe un taux d’autofinancement supérieur à 50 % en situation saine : environ 59 % est donc favorable. Il reste utile d’examiner comment la part non autofinancée sera couverte.'];
 qa('.indicator-row').forEach(row=>{const i=+row.dataset.indicator,fb=row.querySelector('.indicator-feedback');qa('.indicator-pick',row).forEach(b=>{const chosen=state.indicatorQual?.[i]===b.dataset.value;b.classList.toggle('selected',chosen);b.setAttribute('aria-pressed',chosen?'true':'false');b.onclick=()=>{state.indicatorQual=state.indicatorQual||{};state.indicatorQual[i]=b.dataset.value;qa('.indicator-pick',row).forEach(x=>{x.classList.toggle('selected',x===b);x.classList.toggle('correct',x===b&&b.dataset.value===expected[i]);x.classList.toggle('wrong',x===b&&b.dataset.value!==expected[i]);x.setAttribute('aria-pressed',x===b?'true':'false')});fb.className='feedback show '+(b.dataset.value===expected[i]?'ok':'bad');fb.innerHTML=`<strong>${b.dataset.value===expected[i]?'Lecture solide.':'Lecture à nuancer.'}</strong> ${explanations[i]}`;update();save()}})});
...
function renderSim(){const rev=REV[state.simRevenue||0],fin=state.simFinance||'mixte',margin=rev.v-177;$('rev-slider').value=state.simRevenue;text($('sim-rev'),rev.v);text($('sim-fin'),FIN[fin]);qa('#finance-choices .choice').forEach(b=>b.classList.toggle('selected',b.dataset.finance===fin));text($('sim-margin'),margin);text($('sim-margin-note'),'Md · hypothèse à charges et dette constantes');let msg='';if(rev.v===178)msg='Hypothèse prudente : la marge indicative tombe à 1 Md ; interrogez la robustesse des recettes et les marges de financement.';else if(rev.v===187)msg='Hypothèse favorable : la marge indicative atteint 10 Md, sous réserve de vérifier la source et le degré de sécurisation de la recette.';else msg='Scénario central : la marge indicative reste à 5 Md et sert de point de comparaison.';if(fin==='emprunt')msg+=' Le recours accru à l’emprunt ne change pas cette marge, mais doit être rapproché de la trajectoire de dette.';if(fin==='fonds')msg+=' Un financement davantage assuré sur fonds propres doit être rapproché de la trésorerie et de la marge réellement disponible.';html($('sim-reading'),`<h4>Lecture de votre scénario</h4><p>${esc(msg)}</p><p class="muted">Calcul pédagogique : recettes − 157 Md de dépenses hors intérêts − 12 Md d’intérêts − 8 Md de remboursement du capital. Les autres paramètres restent constants ; ce résultat n’est pas une prévision.</p>`);renderScenarios();save()}
function saveScenario(){if(state.scenarios.length>=2)return;const rev=REV[state.simRevenue],fin=state.simFinance,margin=rev.v-177;const sig=`${rev.v}|${fin}`;if(state.scenarios.some(s=>s.sig===sig)){feedback('sim-feedback','bad','<strong>Comparez deux configurations différentes.</strong> Modifiez l’hypothèse de recettes ou le financement avant d’enregistrer le second scénario.');return}state.scenarios.push({sig,rev:rev.v,revLab:rev.lab,finance:FIN[fin],margin});renderScenarios();if(state.scenarios.length===1)$('sim-save').textContent='Conserver le deuxième scénario';else{$('sim-save').disabled=true;$('sim-save').textContent='2 scénarios enregistrés';if(state.simControl)complete('bud-m4-simulateur')}save()}
function renderScenarios(){const markup=[0,1].map(i=>{const s=state.scenarios[i];if(!s)return `<div class="scenario empty">Scénario ${i+1} à construire</div>`;return `<div class="scenario"><div class="counter">Scénario ${i+1}</div><h4>${esc(s.revLab)} · ${esc(s.finance)}</h4><div class="tags"><span class="note-tag">Hypothèse de recettes : ${esc(s.revLab.toLowerCase())}</span><span class="note-tag">Recettes ${s.rev} Md</span><span class="note-tag">Investissements 22 Md</span><span class="note-tag">Marge indicative ${Number.isFinite(s.margin)?s.margin:s.rev-177} Md</span></div></div>`}).join('');['scenario-compare','vigilance-scenarios'].forEach(id=>{if($(id))$(id).innerHTML=markup});if($('sim-save'))$('sim-save').disabled=state.scenarios.length>=2;const ctrl=$('sim-control-block');if(ctrl)ctrl.hidden=state.scenarios.length<2;}
...
const QUESTION_ENJEUX=['Comprendre ce qui explique l’évolution observée','Apprécier la soutenabilité dans le temps','Vérifier le financement d’un engagement'];
const QUESTION_INFOS=['L’hypothèse et sa source','La trajectoire détaillée sur plusieurs exercices','Le calendrier et le degré de sécurisation du financement'];
const QUESTION_WHOS=['Service financier compétent','Commission compétente','Service porteur du projet'];
function questionSourceValues(){
 const perso=[...carnetPriorities(),...carnetVigilances(),...carnetConstats(),...carnetSignals()];
 const choixValides=['#priority-choices .choice[data-ok="1"]','#vig-choices .choice[data-ok="1"]','#constat-choices .choice[data-ok="1"]'].flatMap(sel=>qa(sel).map(b=>b.textContent.trim()));
 const dossier=Object.values(D.DOSSIER||{}).flatMap(d=>(d.options||[]).filter(opt=>opt&&opt[1]===1).map(opt=>opt[0]));
 return Array.from(new Set([...perso,...choixValides,...dossier].map(v=>String(v||'').trim()).filter(Boolean)))
}
function initQuestionBuilder(){
 ...
 const note=$('qb-source-note');if(note)note.textContent=`${vals.length} repères disponibles : vos éléments du carnet et l’ensemble des signaux pertinents déjà travaillés dans le module.`;
...
function currentQuestionRecord(){const signal=$('qb-signal')?.value||'',e=Math.max(0,($('qb-enjeu')?.selectedIndex||0)-1),i=Math.max(0,($('qb-info')?.selectedIndex||0)-1),w=Math.max(0,($('qb-who')?.selectedIndex||0)-1);return [signal,e,i,w]}
function questionRecordValid(q){return Array.isArray(q)&&q.length===4&&typeof q[0]==='string'&&q[0].trim().length>0&&[q[1],q[2],q[3]].every(n=>Number.isInteger(Number(n))&&Number(n)>=0&&Number(n)<=2)}
function questionText(q){if(typeof q==='string')return q;if(!questionRecordValid(q))return '';const e=QUESTION_ENJEUX[Number(q[1])],i=QUESTION_INFOS[Number(q[2])],w=QUESTION_WHOS[Number(q[3])],objectif=e.toLowerCase(),liaison=/^[aeiouyhàâäéèêëîïôöùûü]/i.test(objectif)?'d’':'de ';return `À partir du constat « ${q[0]} », pouvez-vous préciser ${i.toLowerCase()} afin ${liaison}${objectif} ? Interlocuteur pressenti : ${w}.`}
function questionExchangeText(q){return questionText(q).replace(/\s*Interlocuteur pressenti\s*:\s*[^.]+\.?\s*$/i,'').trim()}
function updateQuestionPreview(){const vals=['qb-signal','qb-enjeu','qb-info','qb-who'].map(id=>$(id).value);const ready=vals.every(Boolean);const q=ready?questionText(currentQuestionRecord()):'Complétez les quatre briques pour générer votre formulation.';text($('qb-preview'),q);$('qb-add').disabled=!ready||state.questions.length>=2;$('qb-add').dataset.preview=q}
function resetExchangeIfNeeded(){if(!isComplete('bud-m5-commission')){state.exchangeCurrent=0;state.exchangeSteps=[0,0];state.exchangeMetrics=[{precision:0,instruction:0},{precision:0,instruction:0}];state.exchangeChoices=[[],[]];state.exchangeNotes=[[],[]];state.exchangeDone=false;clearFeedback('exchange-feedback')}}
function addQuestion(){if(state.questions.length>=2||$('qb-add').disabled)return;resetExchangeIfNeeded();const rec=currentQuestionRecord();if(!questionRecordValid(rec))return;state.questions.push(rec);['qb-signal','qb-enjeu','qb-info','qb-who'].forEach(id=>$(id).value='');renderQuestions();updateQuestionPreview();save()}
function renderQuestions(){const box=$('question-cards');if(!box)return;box.innerHTML=state.questions.map((q,i)=>`<div class="card ${i%2?'gold':'info'}"><div class="counter">Question ${i+1}</div><p>${esc(questionText(q))}</p><button class="btn secondary q-remove" data-i="${i}">Retirer / reformuler</button></div>`).join('');qa('.q-remove',box).forEach(b=>b.onclick=()=>{resetExchangeIfNeeded();state.questions.splice(+b.dataset.i,1);renderQuestions();updateQuestionPreview();save()});text($('qb-counter'),`${state.questions.length}/2 questions préparées`);$('question-next').disabled=!questionsPretes();syncBottomNavigation();if(questionsPretes())complete('bud-m5-question')}

const EXCHANGE_STEPS=[
 {choices:[
  {q:2,type:'Donnée',l:'Pouvez-vous préciser la source de cette hypothèse, la méthode d’estimation utilisée et son degré de sécurisation ?',a:'Nous pouvons documenter la source, la méthode et le degré de sécurisation de l’hypothèse. Cela permet de distinguer ce qui est acquis de ce qui reste incertain.',f:'<strong>Question précise.</strong> Vous obtenez une information vérifiable sur la construction de l’hypothèse.'},
  {q:1,type:'Hypothèse',l:'Pouvez-vous nous confirmer si cette donnée est considérée comme sûre ?',a:'Nous pouvons donner une appréciation générale, mais elle reste peu exploitable sans la source, la méthode et le degré de sécurisation.',f:'<strong>Vous progressez.</strong> La demande est utile, mais elle gagnerait à préciser la source et la méthode.'},
  {q:0,type:'Conclusion',l:'Est-ce que cela signifie que le budget est globalement bon ou mauvais ?',a:'Nous pouvons documenter des données et des hypothèses. Un jugement global ne permet pas d’instruire la décision.',f:'<strong>Retour formatif.</strong> Cherchez une donnée ou une hypothèse à documenter plutôt qu’un jugement global.'}
 ],note:'À vérifier : source, méthode et degré de sécurisation de l’hypothèse de recette.'},
 {choices:[
  {q:2,type:'Conséquence',l:'Comment cette réponse se compare-t-elle aux trois exercices précédents, et qu’est-ce qui explique les écarts observés ?',a:'La trajectoire permet de distinguer un événement ponctuel d’une évolution durable. Il faut rapprocher recettes, dépenses, épargne et dette sur plusieurs exercices.',f:'<strong>Bonne relance.</strong> Vous replacez l’information dans la trajectoire avant de conclure.'},
  {q:1,type:'Hypothèse',l:'Pouvons-nous simplement retenir que les recettes devraient finalement augmenter ?',a:'Une évolution attendue peut être utile, mais elle doit être reliée à la trajectoire et aux hypothèses qui la soutiennent.',f:'<strong>À préciser.</strong> Reliez l’évolution attendue aux exercices précédents et à la méthode d’estimation.'},
  {q:0,type:'Donnée',l:'Je préfère passer au montant des investissements sans revenir sur cette réponse.',a:'Changer de sujet à ce stade laisse l’hypothèse insuffisamment instruite.',f:'<strong>Retour formatif.</strong> Exploitez d’abord la réponse obtenue et reliez-la aux autres éléments du dossier.'}
 ],note:'À mettre en perspective : trajectoire pluriannuelle et facteurs expliquant les écarts.'},
 {choices:[
  {q:2,type:'Conclusion provisoire',l:'Si je résume : voici ce qui est acquis, ce qui reste incertain et l’information qu’il nous faut encore obtenir. Est-ce exact ?',a:'Vous disposez alors d’une conclusion de travail : faits acquis, incertitudes identifiées et prochaine information à demander.',f:'<strong>Réflexe transférable.</strong> Vous terminez l’échange par une synthèse exploitable pour la décision.'},
  {q:1,type:'Conclusion provisoire',l:'Pouvez-vous simplement nous confirmer que nous avons désormais tous les éléments ?',a:'Une confirmation générale clôt l’échange, mais elle laisse moins clairement apparaître ce qui reste à instruire.',f:'<strong>À renforcer.</strong> Nommez l’information encore manquante et la conséquence pour votre analyse.'},
  {q:0,type:'Conclusion',l:'Je conclus que le scénario le plus favorable est celui qu’il faut retenir.',a:'Le comparateur ne classe pas les choix politiques. Il sert à révéler les hypothèses et informations à instruire.',f:'<strong>Retour formatif.</strong> Une hypothèse favorable n’est pas, à elle seule, une décision à retenir.'}
 ],note:'Avant de conclure : distinguer les faits acquis, les incertitudes et l’information encore manquante.'}
];
function questionsArrayValid(qs){return Array.isArray(qs)&&qs.length===2&&qs.every(q=>(typeof q==='string'&&q.trim().length>0)||questionRecordValid(q))}
function recoverQuestions(){if(questionsArrayValid(state.questions))return true;const saved=GABARIT.lireDonnee('budget',null);if(saved&&questionsArrayValid(saved.qu)){state.questions=saved.qu;return true}return false}
function questionsPretes(){return recoverQuestions()}
function questionWho(index){const q=state.questions[index];if(questionRecordValid(q))return QUESTION_WHOS[Number(q[3])]||'Service financier compétent';const t=typeof q==='string'?q:'',m=t.match(/Interlocuteur pressenti\s*:\s*([^\.]+)\.?$/i);return m?m[1].trim():'Service financier compétent'}
function ensureExchangeState(){if(!Array.isArray(state.exchangeSteps)||state.exchangeSteps.length!==2)state.exchangeSteps=[0,0];if(!Array.isArray(state.exchangeMetrics)||state.exchangeMetrics.length!==2)state.exchangeMetrics=[{precision:0,instruction:0},{precision:0,instruction:0}];if(!Array.isArray(state.exchangeChoices)||state.exchangeChoices.length!==2||!Array.isArray(state.exchangeChoices[0]))state.exchangeChoices=[[],[]];if(!Array.isArray(state.exchangeNotes)||state.exchangeNotes.length!==2||!Array.isArray(state.exchangeNotes[0]))state.exchangeNotes=[[],[]];state.exchangeCurrent=Math.max(0,Math.min(1,Number(state.exchangeCurrent)||0))}
function exchangeStatus(index){ensureExchangeState();const done=(state.exchangeSteps[index]||0)>=3;if(done)return 'done';if(index===1&&(state.exchangeSteps[0]||0)<3)return 'locked';return 'ready'}
function renderExchangeIntro(){
 const box=$('exchange-intro-cards');if(!box)return;ensureExchangeState();const ready=questionsPretes();
 if(!ready){box.innerHTML='<div class="card info"><strong>Deux questions sont nécessaires.</strong><p>Revenez au constructeur pour préparer vos deux formulations avant de commencer les échanges.</p></div>';const start=$('exchange-intro-start');if(start){start.disabled=true;start.textContent='Préparer mes questions'};return}
 if(!isComplete('bud-m5-echanges-intro'))complete('bud-m5-echanges-intro');
 box.innerHTML=state.questions.map((q,i)=>{const status=exchangeStatus(i),stateText=status==='done'?'Terminé':status==='ready'?'Prêt':'Après l’échange 1',who=questionWho(i);return `<article class="exchange-intro-card ${status}"><div class="exchange-intro-card-head"><span class="exchange-intro-num">Échange ${i+1}</span><span class="exchange-state-pill">${stateText}</span></div><div class="exchange-card-person">${roleAvatar(who,'service-avatar--card')}<div><span>Interlocuteur</span><h3>${esc(who)}</h3></div></div><blockquote>« ${esc(questionExchangeText(q))} »</blockquote></article>`}).join('');
 const start=$('exchange-intro-start');if(start){const s0=exchangeStatus(0),s1=exchangeStatus(1);start.disabled=false;if(s0!=='done'){start.textContent='Commencer l’échange 1';start.onclick=()=>{state.exchangeCurrent=0;clearFeedback('exchange-feedback');go('bud-m5-commission');save()}}else if(s1!=='done'){start.textContent='Commencer l’échange 2';start.onclick=()=>{state.exchangeCurrent=1;clearFeedback('exchange-feedback');go('bud-m5-commission');save()}}else{start.textContent='Voir le bilan des deux échanges';start.onclick=()=>{state.exchangeCurrent=1;clearFeedback('exchange-feedback');go('bud-m5-commission');save()}}}
 const back=$('exchange-intro-back');if(back)back.onclick=()=>go('bud-m5-question');
}
function initExchange(){ensureExchangeState();renderExchange();const end=$('exchange-end'),replay=$('exchange-replay');if(end)end.onclick=()=>{const qi=state.exchangeCurrent;if(state.exchangeSteps[qi]<3)return;if(qi===0){state.exchangeCurrent=1;clearFeedback('exchange-feedback');save();renderExchange();const panel=$('bud-m5-commission');if(panel)panel.scrollIntoView({behavior:'smooth',block:'start'})}else{state.exchangeDone=true;complete('bud-m5-commission');renderCarnet();save();go('m5-close')}};if(replay)replay.onclick=()=>{const qi=state.exchangeCurrent;state.exchangeSteps[qi]=0;state.exchangeMetrics[qi]={precision:0,instruction:0};state.exchangeChoices[qi]=[];state.exchangeNotes[qi]=[];state.exchangeDone=false;clearFeedback('exchange-feedback');renderExchange();renderCarnet();save()}}
function doExchange(q,label,answer,fb){ensureExchangeState();const qi=state.exchangeCurrent,step=state.exchangeSteps[qi];if(!questionsPretes()||step>=3)return;const metrics=state.exchangeMetrics[qi];metrics.precision=Math.min(100,metrics.precision+([8,20,32][q]||8));metrics.instruction=Math.min(100,metrics.instruction+([12,20,30][q]||12));state.exchangeChoices[qi].push(q);state.exchangeNotes[qi][step]=`Question ${qi+1} · ${EXCHANGE_STEPS[step].note}`;state.exchangeSteps[qi]++;feedback('exchange-feedback',q===0?'bad':'ok',fb);renderExchange();renderCarnet();save()}
function roleInitials(role){const known={'Service financier compétent':'SF','Commission compétente':'CO','Service porteur du projet':'SP'};if(known[role])return known[role];const stop=new Set(['de','du','des','la','le','les','l','d','et','à','au','aux']);const parts=String(role||'').replace(/[’']/g,' ').split(/\s+/).filter(Boolean).filter(w=>!stop.has(w.toLowerCase()));return (parts.slice(0,2).map(w=>w[0]).join('')||'IN').toUpperCase()}
function roleAvatar(role,extra=''){const initials=roleInitials(role);return `<div class="service-avatar ${extra}" role="img" aria-label="Interlocuteur : ${esc(role)}"><span>${esc(initials)}</span></div>`}
function serviceBubble(text,who){return `<div class="dialogue-row service-row">${roleAvatar(who)}<div class="dialogue-content"><div class="dialogue-speaker"><span class="speaker-role">${esc(who)}</span><span class="speaker-meta">Interlocuteur</span></div><div class="bubble service">${esc(text)}</div></div></div>`}
function userBubble(text,label='Vous'){return `<div class="dialogue-row user-row"><div><div class="dialogue-speaker">${esc(label)}</div><div class="bubble user">${esc(text)}</div></div></div>`}
function initialExchangeHTML(qi,who){return userBubble(questionExchangeText(state.questions[qi]),'Votre question de départ')+serviceBubble('Voici les éléments dont nous disposons à ce stade. Nous pouvons préciser ce qui est documenté, ce qui relève d’une hypothèse et ce qui reste à vérifier.',who)}
function exchangePairHTML(stepIndex,quality,who){const st=EXCHANGE_STEPS[stepIndex],c=st.choices.find(x=>x.q===quality)||st.choices[0];return userBubble(c.l)+serviceBubble(c.a,who)}
function renderExchangeThread(qi,who){const d=$('dialogue');if(!d)return;if(!questionsPretes()){d.innerHTML=serviceBubble('Préparez vos deux questions avant de commencer.','Interlocuteur');return}const choices=state.exchangeChoices[qi]||[],step=state.exchangeSteps[qi]||0;if(step<=1){d.innerHTML=initialExchangeHTML(qi,who)+choices.map((q,i)=>exchangePairHTML(i,q,who)).join('');return}let older=initialExchangeHTML(qi,who);for(let i=0;i<Math.max(0,choices.length-1);i++)older+=exchangePairHTML(i,choices[i],who);const latest=choices.length?exchangePairHTML(choices.length-1,choices[choices.length-1],who):'';const count=Math.max(1,choices.length);d.innerHTML=`<details class="exchange-history"><summary>✓ ${count} intervention${count>1?'s':''} précédente${count>1?'s':''} · revoir</summary><div class="exchange-history-content">${older}</div></details>${latest}`}
function exchangeSummaryItems(qi){const qualities=state.exchangeChoices[qi]||[],labels=[['La source, la méthode et le degré de sécurisation ont été explicitement demandés.','La construction de l’hypothèse mérite encore d’être précisée.'],['La réponse a été replacée dans une trajectoire pluriannuelle.','La mise en perspective dans le temps reste à renforcer.'],['L’échange se termine par une synthèse de ce qui est acquis, incertain et encore à vérifier.','La conclusion pourrait mieux distinguer acquis, incertitudes et suite à donner.']];return labels.map((pair,i)=>{const strong=(qualities[i]||0)>=2;return `<div class="exchange-summary-item ${strong?'':'todo'}"><span class="mark">${strong?'✓':'○'}</span><span>${esc(pair[strong?0:1])}</span></div>`}).join('')}
function exchangeTranscriptHTML(qi,who){const choices=state.exchangeChoices[qi]||[];return initialExchangeHTML(qi,who)+choices.map((q,i)=>exchangePairHTML(i,q,who)).join('')}
function exchangeOneSummary(qi){const m=state.exchangeMetrics[qi]||{precision:0,instruction:0},who=questionWho(qi),next=1,nextWho=questionWho(next),nextQuestion=questionExchangeText(state.questions[next]);return `<div class="exchange-summary"><div class="exchange-summary-hero"><h3>Ce que cet échange vous a permis de préciser</h3><p>Retenez les acquis, les points encore à renforcer et la suite à donner.</p></div><div class="exchange-metric-delta"><div><span>Précision de l’information</span><b>${Number(m.precision)||0} %</b></div><div><span>Avancement de l’instruction</span><b>${Number(m.instruction)||0} %</b></div></div><div class="exchange-summary-list">${exchangeSummaryItems(qi)}</div><details class="exchange-review"><summary>Revoir la conversation</summary><div class="exchange-review-thread dialogue">${exchangeTranscriptHTML(qi,who)}</div></details><div class="exchange-next-card"><div class="exchange-next-head"><div><span class="exchange-overline">Prochain échange</span><strong>Votre deuxième question est déjà prête</strong></div>${roleAvatar(nextWho,'service-avatar--mini')}</div><div class="exchange-next-who">${esc(nextWho)}</div><blockquote>« ${esc(nextQuestion)} »</blockquote><p>Vous changez de question et, selon votre choix, d’interlocuteur. L’objectif reste le même : préciser ce qui est établi, ce qui reste incertain et ce qui doit encore être instruit.</p></div></div>`}
function exchangeFinalDebrief(){const cards=[0,1].map(i=>{const m=state.exchangeMetrics[i]||{precision:0,instruction:0};return `<div class="exchange-final-card"><div class="exchange-final-person">${roleAvatar(questionWho(i),'service-avatar--mini')}<div><span>Échange ${i+1}</span><h4>${esc(questionWho(i))}</h4></div></div><p>${esc(questionExchangeText(state.questions[i]))}</p><div class="mini-metrics"><span>Précision ${Number(m.precision)||0} %</span><span>Instruction ${Number(m.instruction)||0} %</span></div></div>`}).join('');return `<div class="exchange-final-debrief"><div class="exchange-summary-hero"><h3>Vos deux échanges en un coup d’œil</h3><p>Les deux questions ont été éprouvées et enrichies par des relances successives.</p></div><div class="compare">${cards}</div><div class="exchange-reflex"><strong>Le réflexe à retenir</strong><p>Une question utile ne cherche pas seulement une information supplémentaire : elle aide à distinguer ce qui est établi, ce qui relève d’une hypothèse et ce qui doit encore être instruit.</p></div></div>`}
function renderExchange(){
 if(!$('exchange-options'))return;ensureExchangeState();const qi=state.exchangeCurrent,step=state.exchangeSteps[qi],metrics=state.exchangeMetrics[qi],precision=metrics.precision||0,instruction=metrics.instruction||0,who=questionWho(qi),ready=questionsPretes(),done=ready&&step>=3;
 const panel=$('bud-m5-commission'),dashboard=document.querySelector('#bud-m5-commission .exchange-dashboard'),shell=document.querySelector('#bud-m5-commission .exchange-conversation-shell');if(panel)panel.classList.toggle('exchange-complete',done);if(dashboard)dashboard.classList.toggle('is-complete',done);if(shell)shell.classList.toggle('is-complete',done);
 if($('gauge-precision'))$('gauge-precision').style.width=precision+'%';if($('gauge-progress'))$('gauge-progress').style.width=instruction+'%';text($('gauge-precision-value'),precision+' %');text($('gauge-progress-value'),instruction+' %');if($('gauge-precision-track'))$('gauge-precision-track').setAttribute('aria-valuenow',precision);if($('gauge-progress-track'))$('gauge-progress-track').setAttribute('aria-valuenow',instruction);
 text($('exchange-session-label'),ready?`Échange ${qi+1} sur 2`:'Échanges en attente');text($('exchange-who-label'),ready?who:'Interlocuteur à définir');text($('exchange-counter'),ready?(step<3?`Intervention ${step+1} sur 3`:'Échange terminé'):'En attente');const dashAvatar=$('exchange-dashboard-avatar');if(dashAvatar){dashAvatar.textContent=ready?roleInitials(who):'IN';dashAvatar.setAttribute('aria-label',ready?`Interlocuteur : ${who}`:'Interlocuteur à définir')}
 const active=$('exchange-active-question');if(active){if(!ready)active.innerHTML='<span class="exchange-question-label">Avant de commencer</span><p>Préparez vos deux questions.</p>';else if(done)active.innerHTML=`<span class="exchange-question-label">Question de départ</span><p>${esc(questionExchangeText(state.questions[qi]))}</p>`;else active.innerHTML=`<span class="exchange-question-label">Votre question de départ</span><p>${esc(questionExchangeText(state.questions[qi]))}</p>`}
 const qc=$('exchange-question-context'),prepared=qc?qc.closest('details'):null;if(qc){if(ready){qc.innerHTML=state.questions.map((q,i)=>`<div class="exchange-question ${i===qi?'active':''}"><b>Échange ${i+1} · ${esc(questionWho(i))}</b><p>${esc(questionExchangeText(q))}</p></div>`).join('');if(prepared)prepared.open=false}else{qc.innerHTML='<div class="exchange-question exchange-question-missing"><b>Deux questions sont nécessaires avant de commencer.</b><p>Préparez vos deux formulations.</p><button type="button" class="btn secondary" id="exchange-prepare-questions">Préparer mes questions</button></div>';if(prepared)prepared.open=true;const prep=$('exchange-prepare-questions');if(prep)prep.onclick=()=>go('bud-m5-question')}}
 renderExchangeThread(qi,who);
 const box=$('exchange-options'),head=$('exchange-choice-title'),help=$('exchange-choice-help'),end=$('exchange-end'),replay=$('exchange-replay');box.innerHTML='';if(replay)replay.style.display=ready&&step>0?'inline-flex':'none';if(end)end.style.display='inline-flex';
 if(!ready){if(head)head.textContent='Préparez vos deux questions';if(help)help.textContent='La conversation commencera lorsque les deux formulations seront enregistrées.';box.innerHTML='<div class="card info"><strong>Les deux questions sont nécessaires.</strong><p>Revenez au constructeur avant de poursuivre.</p></div>';if(end){end.disabled=true;end.textContent='Continuer'}}
 else if(step<3){if(head)head.textContent='Que dites-vous maintenant ?';if(help)help.textContent='Choisissez la phrase que vous prononceriez à ce stade.';EXCHANGE_STEPS[step].choices.forEach(c=>{const b=document.createElement('button');b.className='choice';b.dataset.quality=c.q;b.innerHTML=`<span class="exchange-type">${esc(c.type)}</span><br>${esc(c.l)}`;b.onclick=()=>doExchange(c.q,c.l,c.a,c.f);box.appendChild(b)});if(end){end.disabled=true;end.textContent=qi===0?'Passer à l’échange 2':'Terminer'}}
 else{clearFeedback('exchange-feedback');if(head)head.textContent=qi===0?'Premier échange terminé':'Deux échanges terminés';if(help)help.textContent=qi===0?'Regardez ce que vous avez précisé avant de changer de question.':'Comparez les deux échanges avant de poursuivre.';box.innerHTML=qi===0?exchangeOneSummary(qi):exchangeFinalDebrief();if(end){end.disabled=false;end.textContent=qi===0?'Commencer l’échange 2':'Continuer la mission'}}
...
function carnetHTML(){const exchangeNotes=(state.exchangeNotes||[]).flat().filter(Boolean);return `<div class="note-section"><div class="note-label">Mission 1 · Mon plan</div>${tags(carnetPlan())}</div><div class="note-section"><div class="note-label">Mission 2 · Signaux épinglés</div>${tags(carnetSignals())}</div><div class="note-section"><div class="note-label">Mission 3 · Mes constats</div>${tags(carnetConstats())}</div><div class="note-section"><div class="note-label">Mission 4 · Mes vigilances</div>${tags(carnetVigilances())}</div><div class="note-section"><div class="note-label">Mission 5 · Mes priorités</div>${tags(carnetPriorities())}</div><div class="note-section"><div class="note-label">Mes questions préparées</div>${tags((state.questions||[]).map(questionText).filter(Boolean))}</div><div class="note-section"><div class="note-label">Après les deux échanges · À conserver</div>${tags(exchangeNotes)}</div>`}
...
function hydrateScreen(id){
 ...
 if(id==='bud-m4-vigilances')renderScenarios();
 if(id==='bud-m5-question')initQuestionBuilder();
 if(id==='bud-m5-echanges-intro')renderExchangeIntro();
 if(id==='bud-m5-commission')renderExchange();
 ...
 const guards={
  ...
  'bud-m5-echanges-intro':['exchange-intro-cards',renderExchangeIntro],
...
function wire(){
 ...
 if($('qb-add'))$('qb-add').onclick=addQuestion;if($('question-next'))$('question-next').onclick=()=>{if(!questionsPretes())return;clearFeedback('question-transition-feedback');const ok=save();if(!ok){feedback('question-transition-feedback','bad','<strong>Enregistrement incomplet.</strong> Vos deux questions sont bien préparées, mais leur sauvegarde de reprise n’a pas pu être sécurisée. Revenez en arrière puis réessayez avant de poursuivre.');return}ensureExchangeState();go('bud-m5-echanges-intro')};initExchange();if($('m5-view-carnet'))$('m5-view-carnet').onclick=()=>openTool('bud-carnet');
...
const MACRO_NAV=[
 ...
 {id:'m5-intro',kicker:'Mission 5',label:'Préparer ses échanges',pages:['m5-intro','bud-m5-priorites','bud-m5-interlocuteur','bud-m5-question','bud-m5-echanges-intro','bud-m5-commission','m5-close']},
