...
function compactModule(){
 return {v:6,b:state.badges,rs:state.repSeen,rc:state.repCurrent,as:state.actorsSeen,cp:state.cyclePlaced,co:state.cycleOrder,cm:state.cycleMode,pl:state.plan,
...
function addQuestion(){if(state.questions.length>=2||$('qb-add').disabled)return;resetExchangeIfNeeded();state.questions.push($('qb-add').dataset.preview);['qb-signal','qb-enjeu','qb-info','qb-who'].forEach(id=>$(id).value='');renderQuestions();updateQuestionPreview();save()}
...
function questionsPretes(){return Array.isArray(state.questions)&&state.questions.length===2&&state.questions.every(q=>typeof q==='string'&&q.trim().length>0)}
function questionWho(index){const q=state.questions[index]||'',m=q.match(/Interlocuteur pressenti\s*:\s*([^\.]+)\.?$/i);return m?m[1].trim():'Service financier compétent'}
...
function renderExchange(){
 ...
 if(qc){
  if(ready){qc.innerHTML=state.questions.map((q,i)=>`<div class="exchange-question ${i===qi?'active':''}"><b>Question ${i+1} · ${esc(questionWho(i))}</b><p>${esc(q)}</p></div>`).join('');if(prepared)prepared.open=false}
 ...
 if(active)active.innerHTML=ready?`<div class="counter">Simulation ${qi+1} sur 2 · question active</div><h3>${esc(who)}</h3><p>${esc(state.questions[qi])}</p><span class="note-tag">Cette question reste visible pendant tout l’échange</span>`:'<div class="counter">Simulation en attente</div><h3>Préparez vos deux questions</h3>';
...
function carnetHTML(){const exchangeNotes=(state.exchangeNotes||[]).flat().filter(Boolean);return `<div class="note-section"><div class="note-label">Mission 1 · Mon plan</div>${tags(carnetPlan())}</div><div class="note-section"><div class="note-label">Mission 2 · Signaux épinglés</div>${tags(carnetSignals())}</div><div class="note-section"><div class="note-label">Mission 3 · Mes constats</div>${tags(carnetConstats())}</div><div class="note-section"><div class="note-label">Mission 4 · Mes vigilances</div>${tags(carnetVigilances())}</div><div class="note-section"><div class="note-label">Mission 5 · Mes priorités</div>${tags(carnetPriorities())}</div><div class="note-section"><div class="note-label">Mes questions de commission</div>${tags(state.questions||[])}</div><div class="note-section"><div class="note-label">Après les deux simulations · À conserver</div>${tags(exchangeNotes)}</div>`}
...
function wire(){
 ...
 if($('qb-add'))$('qb-add').onclick=addQuestion;if($('question-next'))$('question-next').onclick=()=>{if(questionsPretes())go('bud-m5-commission')};initExchange();if($('m5-view-carnet'))$('m5-view-carnet').onclick=()=>openTool('bud-carnet');
