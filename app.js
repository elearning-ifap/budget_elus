...
function initFlips(){
 ...
 const expected=['watch','risk','watch','risk'];
 const explanations=['La baisse de 22 à 13 Md appelle une vigilance : le niveau doit être rapproché des charges et de la trajectoire.','Les 9 Md restant à financer ne sont ni bons ni mauvais en eux-mêmes : il faut instruire les ressources prévues et leur sécurité.','Le passage de 3,4 à 7,3 ans signale une dégradation de trajectoire qui appelle une vigilance, sans produire à lui seul un verdict.','Un taux de 59 % laisse une part à financer autrement : il faut instruire la combinaison subventions, fonds propres et emprunt.'];
...
function renderScenarios(){const markup=[0,1].map(i=>{const s=state.scenarios[i];return s?`<div class="scenario"><div class="counter">Scénario ${i+1}</div><h4>${esc(s.revLab)} · ${esc(s.finance)}</h4><div class="tags"><span class="note-tag">Recettes ${s.rev} Md</span><span class="note-tag">Investissements 22 Md</span><span class="note-tag">Marge indicative ${Number.isFinite(s.margin)?s.margin:s.rev-177} Md</span></div></div>`:`<div class="scenario empty">Scénario ${i+1} à construire</div>`}).join('');['scenario-compare','vigilance-scenarios'].forEach(id=>{if($(id))$(id).innerHTML=markup});if($('sim-save'))$('sim-save').disabled=state.scenarios.length>=2;}
