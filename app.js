...
function initExchange(){ensureExchangeState();renderExchange();const end=$('exchange-end'),replay=$('exchange-replay');if(end)end.onclick=()=>{const qi=state.exchangeCurrent;if(state.exchangeSteps[qi]<3)return;if(qi===0){state.exchangeCurrent=1;clearFeedback('exchange-feedback');renderExchange();save()}else{state.exchangeDone=true;complete('bud-m5-commission');renderCarnet();go('m5-close');save()}};if(replay)replay.onclick=()=>{const qi=state.exchangeCurrent;state.exchangeSteps[qi]=0;state.exchangeMetrics[qi]={precision:0,instruction:0};state.exchangeChoices[qi]=[];state.exchangeNotes[qi]=[];state.exchangeDone=false;clearFeedback('exchange-feedback');renderExchange();renderCarnet();save()}}
...
function renderExchange(){
 if(!$('exchange-options'))return;ensureExchangeState();const qi=state.exchangeCurrent,step=state.exchangeSteps[qi],metrics=state.exchangeMetrics[qi],precision=metrics.precision||0,instruction=metrics.instruction||0,who=questionWho(qi);
 ...
 const ready=questionsPretes(),qc=$('exchange-question-context'),prepared=qc?qc.closest('details'):null,active=$('exchange-active-question');
 if(qc){
  if(ready){qc.innerHTML=state.questions.map((q,i)=>`<div class="exchange-question ${i===qi?'active':''}"><b>Question ${i+1} · ${esc(questionWho(i))}</b><p>${esc(questionText(q))}</p></div>`).join('');if(prepared)prepared.open=false}
  else{qc.innerHTML='<div class="exchange-question exchange-question-missing"><b>Deux questions sont nécessaires avant de commencer.</b><p>Préparez vos deux formulations : chacune ouvrira sa propre simulation.</p><button type="button" class="btn secondary" id="exchange-prepare-questions">Préparer mes questions</button></div>';if(prepared)prepared.open=true;const prep=$('exchange-prepare-questions');if(prep)prep.onclick=()=>go('bud-m5-question')}
 }
 if(active)active.innerHTML=ready?`<div class="counter">Simulation ${qi+1} sur 2 · question active</div><h3>${esc(who)}</h3><p>${esc(questionText(state.questions[qi]))}</p><span class="note-tag">Cette question reste visible pendant tout l’échange</span>`:'<div class="counter">Simulation en attente</div><h3>Préparez vos deux questions</h3>';
 const d=$('dialogue');if(d){
  d.innerHTML=ready?serviceBubble(`J’ai bien pris connaissance de votre question ${qi+1}. Précisons ensemble ce qui est acquis et ce qui reste à instruire.`,who):serviceBubble('Avant de commencer cet échange, préparez vos deux questions de commission.','Interlocuteur');
  if(ready)(state.exchangeChoices[qi]||[]).forEach((quality,i)=>{const st=EXCHANGE_STEPS[i],c=st.choices.find(x=>x.q===quality)||st.choices[0];d.insertAdjacentHTML('beforeend',userBubble(c.l)+serviceBubble(c.a,who))})
 }
 text($('exchange-counter'),ready?(step<3?`Simulation ${qi+1} · échange ${step+1} sur 3`:`Simulation ${qi+1} terminée`):'Simulation en attente');const end=$('exchange-end');if(end){end.disabled=!ready||step<3;end.textContent=qi===0?'Passer à la question 2':'Terminer la mission 5'}
 const box=$('exchange-options');box.innerHTML='';if(!ready){box.innerHTML='<div class="card info"><strong>Préparez d’abord vos deux questions.</strong><p>La simulation démarrera automatiquement lorsque les deux formulations seront enregistrées.</p></div>'}
 else if(step<3){EXCHANGE_STEPS[step].choices.forEach(c=>{const b=document.createElement('button');b.className='choice';b.dataset.quality=c.q;b.innerHTML=`<span class="exchange-type">${esc(c.type)}</span><br>${esc(c.l)}`;b.onclick=()=>doExchange(c.q,c.l,c.a,c.f);box.appendChild(b)})}
 else{box.innerHTML=`<div class="card success"><strong>Simulation ${qi+1} terminée.</strong><p>Les trois repères associés à cette question ont rejoint votre carnet. ${qi===0?'Passez maintenant à la seconde question.':'Vous avez travaillé vos deux questions de commission.'}</p></div>`}
}
...
function carnetHTML(){const exchangeNotes=(state.exchangeNotes||[]).flat().filter(Boolean);return `<div class="note-section"><div class="note-label">Mission 1 · Mon plan</div>${tags(carnetPlan())}</div><div class="note-section"><div class="note-label">Mission 2 · Signaux épinglés</div>${tags(carnetSignals())}</div><div class="note-section"><div class="note-label">Mission 3 · Mes constats</div>${tags(carnetConstats())}</div><div class="note-section"><div class="note-label">Mission 4 · Mes vigilances</div>${tags(carnetVigilances())}</div><div class="note-section"><div class="note-label">Mission 5 · Mes priorités</div>${tags(carnetPriorities())}</div><div class="note-section"><div class="note-label">Mes questions de commission</div>${tags((state.questions||[]).map(questionText).filter(Boolean))}</div><div class="note-section"><div class="note-label">Après les deux simulations · À conserver</div>${tags(exchangeNotes)}</div>`}
