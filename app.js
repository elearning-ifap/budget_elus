...
function syncBottomNavigation(){const next=$('g-btn-suiv');if(!next)return;const p=(GABARIT.etat||{}).page;if(!SEQ.includes(p))return;if(p==='g-post'){next.disabled=Object.keys(GABARIT.etat.post||{}).length<(D.COMP||[]).length;return}if(p==='bud-m5-question'){next.disabled=!questionsPretes();return}next.disabled=!isComplete(p)}
...
function renderExchangeIntro(){
 ...
 box.innerHTML=state.questions.map((q,i)=>{const status=exchangeStatus(i),stateText=status==='done'?'Terminé':status==='ready'?'Prêt':'Après l’échange 1';return `<article class="exchange-intro-card ${status}"><div class="exchange-intro-card-head"><span class="exchange-intro-num">Échange ${i+1}</span><span class="exchange-state-pill">${stateText}</span></div><h3>${esc(questionWho(i))}</h3><blockquote>« ${esc(questionExchangeText(q))} »</blockquote><div class="exchange-card-who">Interlocuteur : <b>${esc(questionWho(i))}</b></div></article>`}).join('');
...
function initExchange(){ensureExchangeState();renderExchange();const end=$('exchange-end'),replay=$('exchange-replay');if(end)end.onclick=()=>{const qi=state.exchangeCurrent;if(state.exchangeSteps[qi]<3)return;if(qi===0){state.exchangeCurrent=1;clearFeedback('exchange-feedback');save();go('bud-m5-echanges-intro')}else{state.exchangeDone=true;complete('bud-m5-commission');renderCarnet();save();go('m5-close')}};if(replay)replay.onclick=()=>{const qi=state.exchangeCurrent;state.exchangeSteps[qi]=0;state.exchangeMetrics[qi]={precision:0,instruction:0};state.exchangeChoices[qi]=[];state.exchangeNotes[qi]=[];state.exchangeDone=false;clearFeedback('exchange-feedback');renderExchange();renderCarnet();save()}}
...
function serviceBubble(text,who){return `<div class="dialogue-row service-row"><div class="service-avatar" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><circle cx="12" cy="8" r="4"></circle><path d="M4.8 20c.8-4.1 3.2-6.2 7.2-6.2s6.4 2.1 7.2 6.2"></path></svg></div><div><div class="dialogue-speaker">${esc(who)}</div><div class="bubble service">${esc(text)}</div></div></div>`}
...
function exchangeOneSummary(qi){const m=state.exchangeMetrics[qi]||{precision:0,instruction:0};return `<div class="exchange-summary"><div class="exchange-summary-hero"><h3>Ce que cet échange vous a permis de préciser</h3><p>Vous avez fait évoluer l’instruction de votre question en trois interventions successives.</p></div><div class="exchange-metric-delta"><div><span>Précision de l’information</span><b>0 → ${Number(m.precision)||0} %</b></div><div><span>Avancement de l’instruction</span><b>0 → ${Number(m.instruction)||0} %</b></div></div><div class="exchange-summary-list">${exchangeSummaryItems(qi)}</div>${qi===0?'<div class="exchange-reflex"><strong>Vous avez encore une deuxième question à éprouver.</strong><p>Le prochain échange change de question et peut aussi changer d’interlocuteur.</p></div>':''}</div>`}
function exchangeFinalDebrief(){const cards=[0,1].map(i=>{const m=state.exchangeMetrics[i]||{precision:0,instruction:0};return `<div class="exchange-final-card"><h4>Échange ${i+1} · ${esc(questionWho(i))}</h4><p>${esc(questionExchangeText(state.questions[i]))}</p><div class="mini-metrics"><span>Précision ${Number(m.precision)||0} %</span><span>Instruction ${Number(m.instruction)||0} %</span></div></div>`}).join('');return `<div class="exchange-final-debrief"><div class="exchange-summary-hero"><h3>Vos deux échanges en un coup d’œil</h3><p>Les deux questions ont été éprouvées et enrichies par des relances successives.</p></div><div class="compare">${cards}</div><div class="exchange-reflex"><strong>Le réflexe à retenir</strong><p>Une question utile ne cherche pas seulement une information supplémentaire : elle aide à distinguer ce qui est établi, ce qui relève d’une hypothèse et ce qui doit encore être instruit.</p></div></div>`}
function renderExchange(){
 if(!$('exchange-options'))return;ensureExchangeState();const qi=state.exchangeCurrent,step=state.exchangeSteps[qi],metrics=state.exchangeMetrics[qi],precision=metrics.precision||0,instruction=metrics.instruction||0,who=questionWho(qi),ready=questionsPretes();
 ...
 text($('exchange-session-label'),ready?`Échange ${qi+1} sur 2`:'Échanges en attente');text($('exchange-who-label'),ready?who:'Interlocuteur à définir');text($('exchange-counter'),ready?(step<3?`Intervention ${step+1} sur 3`:'Échange terminé'):'En attente');
 const active=$('exchange-active-question');if(active)active.innerHTML=ready?`<span class="exchange-question-label">Votre question de départ</span><p>${esc(questionExchangeText(state.questions[qi]))}</p>`:'<span class="exchange-question-label">Avant de commencer</span><p>Préparez vos deux questions.</p>';
